...
function shouldIgnoreErrorByStacktrace(pageError: Error): boolean {
  if (pageError.stack) {
    return stackTraceErrorsToIgnore.some((fragment) =>
      pageError.stack?.includes(fragment)
    );
  }
  return false;
}
...
function shouldIgnoreErrorByMessage(pageError: Error): boolean {
  if (pageError?.message) {
    return messageErrorsToIgnore.some((fragment) =>
      pageError.message.includes(fragment)
    );
  }
  return false;
}
...
export const reportPageErrors: StoryWithProps<CustomWorld> = async (
  p,
  world
) => {
