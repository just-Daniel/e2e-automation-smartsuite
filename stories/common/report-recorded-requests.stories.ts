...
import type { StoryWithProps, Request } from "playwright-fluent";
...
function shouldKeepRecordedRequest(request: Request): boolean {
  ...
  const mustBeIgnored = urlsToBeIgnored.some((urlToBeIgnored) =>
    url.includes(urlToBeIgnored)
  );
  return !mustBeIgnored;
}

export const reportRecordedRequests: StoryWithProps<CustomWorld> = async (
  p,
  world
) => {
  const recordedRequests = p.getRecordedRequestsTo("/");
  const fileteredRequests = recordedRequests
    .filter(shouldKeepRecordedRequest)
    .slice(-10);
